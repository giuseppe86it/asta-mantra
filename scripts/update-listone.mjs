...
// Fantacalcio espone ora il ruolo direttamente sulla riga giocatore,
// es. data-filter-role-mantra="pc". Gestiamo anche eventuali pluriruolo
// separati da / , ; | spazio o trattino.
...
function rowName($row,$){
  // È il dato più stabile emerso dalla diagnostica.
  const fromFilter=clean($row.attr("data-filter-keywords"));
...
function parseRows(html,previous){
  ...
  $("tr.player-row, table tbody tr, table tr").each((_,tr)=>{
    const $row=$(tr);
    // Evita duplicati dovuti al selettore multiplo.
    if(!$row.is("tr.player-row") && $row.closest("table").length && $row.attr("data-index")==null)return;
...
async function readSource(){
  ...
  const res=await fetch(SOURCE_URL,{
    headers:{
      "user-agent":"Mozilla/5.0 (compatible; AstaMantraListoneSync/1.1; +https://github.com/giuseppe86it/asta-mantra)",
...
const retiredKeys=new Set(retired.map(p=>p.key||norm(p.name)));
const olderRetired=(previous.players||[])
  .filter(p=>p.active===false && !activeByKey.has(p.key||norm(p.name)))
  .filter(p=>!retiredKeys.has(p.key||norm(p.name)));
...
console.log(`OK: ${active.length} attivi · ${retired.length} usciti · ${roleParsed} ruoli Mantra letti dalla pagina · ${teams.size} club`);
