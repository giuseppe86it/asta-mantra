...
function rowMarksPlayerOut($row,name){
  const text=clean($row.text());
  const pos=text.indexOf(name);
  if(pos<0)return false;
  return /^\s*\*/.test(text.slice(pos+name.length));
}
...
function parseRows(html, previous){
  ...
  $("table tbody tr, table tr").each((_,tr)=>{
    ...
    const outMarker=rowMarksPlayerOut($row,name);
    ...
    rows.push({
      ...
      strategic:!!prev?.strategic,active:!outMarker,
...
const markedRetired=parsed.filter(p=>p.active===false);
